...
void YOLO::drawPred(float conf, int left, int top, int right, int bottom, Mat& frame, vector<int> landmark)   // Draw the predicted bounding box
{
	...
	for (int i = 0; i < 4; i++)
	{
		circle(frame, Point(landmark[2 * i], landmark[2 * i + 1]), 2, Scalar(0, 255, 0), -1);
	}
}
...
void YOLO::detect(Mat& frame)
{
	...
	int n = 0, q = 0, i = 0, j = 0, row_ind = 0, k = 0; ///xmin,ymin,xamx,ymax,box_score,x1,y1, ... ,x4,y4,plate_score
		...
		for (q = 0; q < 3; q++)    ///anchor
		{
			...
			for (i = 0; i < num_grid_y; i++)
			{
				for (j = 0; j < num_grid_x; j++)
				{
					...
					{
						float plate_score = sigmoid_x(pdata[13]);
						...
						confidences.push_back(plate_score);
						boxes.push_back(Rect(left, top, (int)(w*ratiow), (int)(h*ratioh)));
						vector<int> landmark(8);
						for (k = 5; k < 13; k += 2)
						{
							const int ind = k - 5;
							landmark[ind] = int((pdata[k] * anchor_w + j * this->stride[n] - padw) * ratiow);
							landmark[ind + 1] = int((pdata[k + 1] * anchor_h + i * this->stride[n] - padh) * ratioh);
						}
